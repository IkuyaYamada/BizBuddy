...
export const useHierarchicalTasks = (tasks: Task[]) => {
  ...
  // タスクの取得とソート
  const sortMainTasks = useCallback(
    async (mainTasks: HierarchicalTask[]): Promise<HierarchicalTask[]> => {
  ...
  const updateTaskOrders = useCallback(
    async (taskId: number, newOrder: number) => {
      try {
  ...
  // タスクをツリー構造に整理する関数
  const organizeTasksIntoTree = useCallback(
    (tasks: HierarchicalTask[]): HierarchicalTask[] => {
      // 親IDでタスクをグループ化
      ...
      Object.entries(tasksByParent).forEach(([parentId, group]) => {
        group.sort((a, b) => {
          ...
          const orderA =
            statusOrder[statusA as keyof typeof statusOrder] ?? 999;
          const orderB =
            statusOrder[statusB as keyof typeof statusOrder] ?? 999;
  ...
  const addTask = useCallback(
    async (parentId?: number, level: number = 0) => {
      try {
        // 同じ親を持つタスクの中で最大のposition値を取得
        const siblingTasks = hierarchicalTasks.filter(
          (t) => t.parent_id === parentId
        );
        const maxPosition = siblingTasks.reduce(
  ...
  const updateTask = useCallback(
    async (taskId: number, updates: Partial<HierarchicalTask>) => {
      try {
        ...
        // APIを呼び出し
        const updatedTask = await api.updateHierarchicalTask(
          taskId,
          apiUpdates
        );
